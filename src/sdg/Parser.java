...
/**
 *
 * @author DeadTomGC (Daniel Coombe)
 */
public class Parser
{
   ...
   public void parseAndRender(ArrayList<Link> links, ArrayList<Module> modules)
   {
      ...
      for (int i = 0; i < links.size(); i++)
      {
         ...
         {
            link1.sizeY = textDim[1] + loopHeight + (thickness * 6);
         }
         else
         {
            link1.sizeY = textDim[1] + (thickness * 6);
         }
         ...
         if (link1.right.index > link1.left.index)
         {
            if (modules.size() > link1.left.index + 1)
            {
               ...
               diff = 0;
               if (tempMod.x+tempMod.sizeX/2 + spacing * 2 + textDim[0] + thickness > link1.right.x+link1.right.sizeX/2)
               {
                  diff = thickness + tempMod.x+tempMod.sizeX/2 + spacing * 2 + textDim[0] - (link1.right.x+link1.right.sizeX/2);
         ...
         {
            if (modules.size() > link1.right.index + 1)
            {
               tempMod = modules.get(link1.right.index + 1);
               diff = 0;
               if (tempMod.x+tempMod.sizeX/2 < thickness + link1.right.x+link1.right.sizeX/2 + spacing * 2 + textDim[0])
               {
                  diff = thickness + link1.right.x+link1.right.sizeX/2 + spacing * 2 + textDim[0] - (tempMod.x+tempMod.sizeX/2);
      ...
      for (int i = 0; i < links.size(); i++)
      {
         ...
         if (link1.left.index == link1.right.index)
         {
            ...
            drawer.drawHook(link1.x - spacing, link1.y + textDim[1] + (thickness * 3) , link1.sizeX + spacing, loopHeight, thickness, radius, false);
         ...
         else
         {
            ...
            if (link1.left.index > link1.right.index)
            {
               drawer.drawArrow(link1.left.x + link1.left.sizeX / 2 - thickness / 2, link1.y + textDim[1] + (thickness * 3), (link1.left.x + link1.left.sizeX / 2) - (link1.right.x + link1.right.sizeX / 2) - thickness, thickness, true);
            }
            else
            {
               drawer.drawArrow(link1.left.x + link1.left.sizeX / 2 + thickness / 2, link1.y + textDim[1] + (thickness * 3), (link1.right.x + link1.right.sizeX / 2) - (link1.left.x + link1.left.sizeX / 2) - thickness, thickness, false);
            }
         }
      }
   }
   ...
   private int[] getStringDim(String text)
   {
      ...
      int retVal[] =
      {
         longest, lines * drawer.g.getFontMetrics().getHeight()
      };
      return retVal;
   }
